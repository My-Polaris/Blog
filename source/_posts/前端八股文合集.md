...
第一次握手，服务端确认自己能收到客户端的请求(能收)；第二次握手，客户端确认自己发送的请求服务端能收到(能发) && 自己能收到服务器的请求(能收)；第三次握手，服务端确认自己发送的响应客户端能收到(能发)，至此，完成确认。
...
setTimeout(function(){// setTimeout实际上是window在调用,所以callback的this永远指向window
    func2.call(this);// 117
},0)
```

### 24.要注意Promise中then的核心
...
```javascript
const set = new Set(); set.add('test');
const map = new Map(); map.set('test', 'test');
...
const arr = [undefined, NaN, Infinity, set, map, regexp, symbol, normalFunction, arrowFunction, date];
...
```javascript
const isObject = (val)=> {// 判断是否是object类型,js的基本类型有:Number,String,Boolean,undeinfed,null,object,Symbol,BigInt
