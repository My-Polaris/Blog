...
浏览器缓存方式依赖服务端响应头cache-control字段。
决定浏览器走什么缓存依赖cache-control字段，有max-age值的时候走强缓存，为no-cache可能走协商缓存，为no-store就是不缓存。

强缓存：过期前都直接返回本地缓存，过期后开始走协商缓存。

协商缓存：根据etag和last-modified判断资源是否发生变化，没发生变化返回304，发生变化则返回200与新资源。
...
Vue的视图更新依赖响应式变量监听，React的视图更新依赖用户主动调用钩子，所以：

- 初始化：React 更快（无变量追踪开销）
- 写入：React 更快（setState 比 Proxy setter 轻） 
- 更新（render + diff）：Vue 更快（精准更新指定组件 vs 更新所有子组件，可memo缓存优化）
...
### 完整的描述一下Vue响应式原理的全过程
分为三个阶段：数据劫持 -> 依赖建联(订阅) -> 视图更新(发布)
以Vue2为例
1. 数据劫持：new Vue初始化时，对data、props、computed等数据做遍历，用Object.definedProperty把它们变为带getter和setter的响应式变量，并给每一个响应式变量配备一个Dep实例，用于存储它们的watcher。
2. 依赖建联（订阅）：mounted时，当组件渲染用到对应的响应式变量时，会触发getter逻辑，getter里将watcher放到该变量的Dep实例中，同时Dep也会被记到watcher中。
3. 视图更新（发布）：当响应式变量的值发生变化时，触发setter逻辑，将Dep里的watcher都拉出来，调用他们的update方法（注意watcher的update方法是异步的），在此次异步周期的update都调完后watcher会调用run方法重新渲染组件
（Vue2因Object.definedProperty导致「响应式对象新增属性」和「索引修改数组元素值、修改数组长度」无法被监听到，$.set则是补充方案，内部对于数组的值改变用了splice代替触发，对于对象的值会判断是否为新增属性，是的话则补充监听）
...
React是all in js，类组件的思想更多像一个有自己状态和生命周期的对象，函数式组件更像输入props输出JSX。函数式编程让编程的关注点更多以逻辑触发，而不是状态出发，是一个趋势。

Suspence组件就是让内部组件还没加载时先展示指定加载态的组件。

ErrorBounday组件就是让内部组件报错时展示兜底态的组件。

React分为五种类型的hook

- 状态钩子：useState、useReducer。用于更新状态，触发更新视图
- 副作用钩子：useEffect。用于连接外部操作，比如浏览器API
- 上下文钩子：useContext。用于传递数据
- 引用钩子：use Ref。用于引用HTML元素
- 性能钩子：useMemo、useCallback。用于避免不必要的操作提升性能
- 过渡钩子：useTransition（延缓组件渲染）、useDeferredValue。处理过渡效果
- React 19钩子：useFormStatus、useFormState、useOptimistic。
- 其他钩子：useId。返回该组件的唯一ID，用于标识唯一组件
...
> useEffect(fn, array)
>
> 设计初衷其实是希望在指定情况下做一些React外部操作的更新，比如监听变量的改变进行document的一些特殊操作。
>
> 这里想提一下vue里watch的用法，其实vue的watch里也应该少用，按照react的优雅思想，所有依赖dom event后进行的操作都应该在事件回调中主动调用，而不是靠到处的watch去监听，以至于你找不到这个操作来源于哪个watch
...
vue和react的最小视图更新单位都是组件，不同于vue的watcher感知哪些组件依赖了这个变量进行按需更新，React只能在调用setState后，更新state往下的所有子组件（当然这里的更新是虚拟dom更新，实际渲染成本是一样的，都是diff出来后只更新变化的dom结点）
...
// 在了解原型与原型链之后，我们知道所有实例都有一个__proto__属性，指向其构造函数的prototype，实例找不到的方法都会去这里找,还找不到会继续按照__proto__找（函数也有__proto__，指向其父类的prototype），比如x=1;x.toFixed本质就是调用的Number.prototype里的toFixed方法
// 接着引申出手写instance of，本质是不断的去找__proto__，看原型链上是否有所找对象的prototype；手写new，本质是用Object.create创建一个空对象，并让这个空对象的__proto__指向构造函数的prototype
// 在了解了手写继承之后，我们知道，手写组合寄生继承就是让子函数的prototype为一个空实例，这个实例的__proto__指向父函数的prototype，这样就接上了原型链；而且在子函数调用时会用call执行一遍父函数，这样子函数调用时也会初始化父函数的一些变量

// 手写ES6 Class（本质ES5构造函数+原型链的语法糖）
// 1. 区分实例方法、实例属性、原型方法和静态方法，前两者挂载到实例的this对象上即可，原型方法要挂载到函数.prototype上，静态方法则直接挂在到函数上
// 2. extends和super关键字封装，extends就是手写继承，super就是调用父类构造函数
